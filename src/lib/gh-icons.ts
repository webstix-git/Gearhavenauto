...
const socialIcon = (
  ...
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="${color}" aria-hidden="true"><path d="${path}"/></svg>`;
};
...
export const GH_ICON_YOUTUBE = (opts: { size?: number; color?: string } = {}) =>
  socialIcon(
